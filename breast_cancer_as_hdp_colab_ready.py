...
WDBC_COLS = ["id", "diagnosis"] + [f"f{i}" for i in range(30)]
raw = pd.read_csv(DATA_URL, header=None, names=WDBC_COLS)

diag = raw["diagnosis"].astype(str).str.strip().str.upper()
diag_map = {"M": 1, "B": 0}
raw["diagnosis"] = diag.map(diag_map)
raw = raw.dropna(subset=["diagnosis"]).copy()
raw["diagnosis"] = raw["diagnosis"].astype(int)

df = clean_df_numeric_only(raw, keep_cols_as_is=["diagnosis"])
if len(df) == 0:
    raise ValueError("Breast cancer dataset loaded but resulted in 0 rows after parsing.")

y = df["diagnosis"].values.astype(np.int64)
if len(np.unique(y)) < 2:
    raise ValueError(f"Need 2 classes for stratified split, but got classes={np.unique(y)}.")

X = df.drop(columns=["id", "diagnosis"]).values.astype(np.float32)
print("Loaded Breast Cancer WDBC | N =", len(y), "| pos(M) =", int(np.sum(y == 1)), "| neg(B) =", int(np.sum(y == 0)))
...
run_experiments("Breast Cancer Wisconsin Diagnostic", X_train, y_train, X_val, y_val, X_test, y_test, d_in, "breast_cancer")
