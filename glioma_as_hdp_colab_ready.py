...
GLIOMA_CSV_PATH = "/content/Glioma.csv"
ALTERNATIVE_PATH = "/content/glioma_grading.csv"
TARGET_COLUMN_NAME = None
TARGET_MAPPING = None
ID_LIKE_COLUMNS = {"id", "patient_id", "case_id", "subject", "sample", "name"}

def guess_target_column(df: pd.DataFrame) -> str:
    candidates = ["target", "label", "class", "Class", "LABEL", "diagnosis", "Diagnosis", "grade", "Grade", "glioma_grade", "WHO_grade", "tumor_grade", "y", "outcome", "Outcome"]
    for c in candidates:
        if c in df.columns:
            return c
    return df.columns[-1]

def map_to_binary(y_raw: pd.Series) -> np.ndarray:
    s = y_raw.copy()
    if isinstance(TARGET_MAPPING, dict):
        mapped = s.astype(str).str.strip().map(TARGET_MAPPING)
    ...
    try:
        ...
        if len(uniq) == 2 and set(uniq).issubset({0, 1}):
            return sn.astype(int).to_numpy()
        if len(uniq) >= 2:
            return (sn.astype(float) >= 3.0).astype(int).to_numpy()
    except Exception:
        pass
    st = s.astype(str).str.strip().str.lower()
    high_keys = ["hgg", "high", "gbm", "glioblastoma", "grade iv", "grade 4", "grade iii", "grade 3"]
    low_keys = ["lgg", "low", "grade ii", "grade 2", "grade i", "grade 1"]
    y = np.full(len(st), np.nan, dtype=np.float64)
    ...
    if np.isnan(y).any():
        ...
        if len(uniques) == 2:
            ...
            raise ValueError(f"Target has {len(uniques)} classes: {list(uniques[:10])}. Provide TARGET_MAPPING to convert it to binary.")
    return y.astype(int)

if os.path.exists(GLIOMA_CSV_PATH):
    path = GLIOMA_CSV_PATH
elif os.path.exists(ALTERNATIVE_PATH):
    path = ALTERNATIVE_PATH
else:
    raise FileNotFoundError("Upload Glioma.csv or glioma_grading.csv to /content, or update GLIOMA_CSV_PATH.")

raw = pd.read_csv(path)
if TARGET_COLUMN_NAME is None:
    TARGET_COLUMN_NAME = guess_target_column(raw)

drop_cols = [c for c in raw.columns if str(c).strip().lower() in ID_LIKE_COLUMNS]
df0 = raw.drop(columns=drop_cols, errors="ignore").copy()
y = map_to_binary(df0[TARGET_COLUMN_NAME])
df0[TARGET_COLUMN_NAME] = y
df = clean_df_numeric_only(df0, keep_cols_as_is=[TARGET_COLUMN_NAME])

if len(df) == 0:
    raise ValueError("Glioma dataset loaded but resulted in 0 rows after preprocessing.")
y = df[TARGET_COLUMN_NAME].values.astype(np.int64)
if len(np.unique(y)) < 2:
    raise ValueError(f"Need 2 classes after mapping, but got classes={np.unique(y)}.")
X = df.drop(columns=[TARGET_COLUMN_NAME]).values.astype(np.float32)

print("Loaded Glioma dataset | path =", path, "| target =", TARGET_COLUMN_NAME, "| features =", X.shape[1], "| N =", len(y), "| pos =", int(np.sum(y == 1)), "| neg =", int(np.sum(y == 0)), "| dropped_id_cols =", drop_cols)
...
run_experiments("Glioma Grading", X_train, y_train, X_val, y_val, X_test, y_test, d_in, "glioma")
